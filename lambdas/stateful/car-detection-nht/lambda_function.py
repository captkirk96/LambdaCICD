...
def detect_cars_in_image(image_path):
    ...
    try:
        ...
        results = []  # Placeholder since detection is handled externally
...
def process_s3_event(s3_event):
    ...
    try:
        ...
        vehicle_status = "No vehicles detected" if not vehicle_results else vehicle_results
        ...
        return {"message": "Output data successfully saved in MongoDB"}
    except Exception as e:
        print(f"DEBUG: Error in Lambda function - {e}")
        return {"error": str(e), "message": "Error processing the event."}
